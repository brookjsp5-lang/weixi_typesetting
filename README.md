...
TypeZen 是一款面向微信公众号创作者的在线排版工作台。你可以把 Markdown 草稿放进编辑区，使用 AI 优化排版结构，套用 96 套微信公众号兼容模板，并在发布前生成封面图、公众号贴图和发布检查清单。
...
- **96 套精美模板**：覆盖新粗野、极简、商务、文艺、科技、节庆、好味手帐、宠物贴贴社 8 大风格分类；其中好味手帐适合探店与食谱记录，宠物贴贴社提供明显活泼的萌宠贴纸感。
...
- **公众号贴图生图**：支持用户输入自由文本，并提供 `工具叠字` 与 `模型直出` 两种模式。`工具叠字` 让模型只生成无文字竖版背景，再由 TypeZen 合成用户提供的文字；`模型直出` 让模型直接生成完整带字贴图。
- **贴图文字样式面板**：在工具叠字模式下可调整卡片位置、宽度、透明度、字号、文字颜色、描边色、强调色和阴影开关，修改样式会基于原背景本地重绘。
...
文本类能力用于排版优化和发布方向生成；图像类能力用于封面和贴图生成。API Key 只保存在当前浏览器本地，调用时会临时发送到服务端代理请求对应模型服务，项目不会持久化保存你的 API Key。

## 贴图生图模式

### 工具叠字

推荐默认模式。模型只负责生成无文字竖版背景，提示词会明确禁止文字、数字、Logo、水印、UI 卡片和信息图。TypeZen 使用 canvas 合成用户输入的贴图文字和样式，适合中文准确性要求较高的公众号贴图。

### 模型直出

高级模式。提示词会要求模型完整显示用户输入的贴图文字，并强调不要改字、漏字或生成乱码。这个模式适合用户希望模型直接完成构图和文字设计的场景，但文字准确性仍取决于模型能力。
