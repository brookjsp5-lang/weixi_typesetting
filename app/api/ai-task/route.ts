...
const IMAGE_ASSIST_PROMPT = `你是微信公众号视觉编辑。请根据用户提供的 Markdown 文章，生成封面和正文配图方向。

只输出一个 JSON 对象，不要解释，不要使用 Markdown 代码块。JSON 结构必须是：
{
  "coverPrompt": "适合生成公众号封面的中文提示词",
  "articleImagePrompts": ["正文配图提示词1", "正文配图提示词2", "正文配图提示词3"],
  "imageDescriptions": ["图片描述1", "图片描述2", "图片描述3"],
  "insertSuggestions": ["建议插入位置1", "建议插入位置2"]
}

要求：
1. 不生成真实图片，不输出图片链接。
2. 所有提示词必须贴合原文主题，不编造原文没有的关键事实。
3. 风格适合微信公众号，画面描述清晰、克制、可执行。
4. 图片描述可直接作为 Markdown 图片 alt 文案。`;
...
const getSystemPrompt = (taskType: AiTaskType, rewritePrompt: string) => {
  ...
  if (taskType === "imageAssist") return IMAGE_ASSIST_PROMPT;
...
export async function POST(req: Request) {
  try {
    ...
    const result = streamText({
      ...
      prompt: markdown,
      temperature: selectedTask === "format" ? 0.3 : selectedTask === "imageAssist" ? 0.7 : 0.5,
    });
